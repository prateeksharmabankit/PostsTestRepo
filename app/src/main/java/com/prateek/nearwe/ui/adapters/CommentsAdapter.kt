...
class CommentsAdapter(context: Context, private val commentsModelList: MutableList<CommentRequest>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == VIEW_TYPE_ONE) {
            return View1ViewHolder(
                LayoutInflater.from(context).inflate(R.layout.recyclerview_comments_row, parent, false)
            )
        }
        return View2ViewHolder(
            LayoutInflater.from(context).inflate(R.layout.recyclerview_comments_right_row, parent, false)
        )
    }
